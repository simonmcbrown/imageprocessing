...
if (SAVEPLOT) postscript(file=sub('.txt','_linear.ps',st1),horiz=F) else x11()
par(mfrow=c(3,2))
#par(mar=c(3,3,2,1))
#par(mgp=c(2,1,0))
...
points(VD.comp,test2,col=6)  ### this is it!!!!!!!
  ...
  plot(VDens51,L.Cal.ref.51,main='Reference-L, Reduced-L, Printed-L')
points(VDens51,L.print.corrected,col=3)
sm.lL.print.corrected <- smooth.spline(log(L.print.corrected),spar=0.4)$y
lines(VDens51,exp(sm.lL.print.corrected))# smooth L.print.corrected
points(VD.comp,exp(lL.comp),col=2) # compressed L values an VDens values
points(VD.comp,exp(test2),col=6)  ### this is it!!!!!!!
grid(nx=50,col='darkgrey',lty=1)
# notes
# lL.comp are the compressed L values
dev.off()
